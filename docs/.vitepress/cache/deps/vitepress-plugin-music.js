...
var x = (a, r) =>
  new Promise((e, l) => {
    ...
    let i = false,
      c = 30;
    const m = () => {
      (a.volume = Math.min(Math.max(a.volume + (r ? c : -c) / 1e3, 0), 1)),
        (i = a.volume === 1 || a.volume === 0),
        i
          ? e(a.volume)
          : ((c = Math.max(c - 1, 5)), (g = requestAnimationFrame(m)));
    };
...
var H = defineComponent({
  ...
  setup(a) {
    const r = a,
      ...
      l = computed(() => r.list.filter((t) => !t.hide)),
      i = computed(() => e.value.status === 0),
      c = computed(() => e.value.status === 1),
      m = computed(() => e.value.status === 2),
      P = computed(() => ({
        'is-play': i.value,
        'is-pause': c.value,
        'is-stop': m.value,
      ...
      F = computed(() => l.value[e.value.currentIndex]),
      k = () => {
      ...
      u = ref(),
      d = (t) => {
        if (t === e.value.currentIndex) return k();
        (e.value.status = 2),
          (e.value.currentIndex =
            t < 0 ? l.value.length - 1 : t > l.value.length - 1 ? 0 : t),
          nextTick(() => (e.value.status = 0));
      },
      N = () => {
              ...
              createBaseVNode('div', E, [
                createBaseVNode('div', T, [
                  ...
                  createBaseVNode(
                    ...
                    'Total ' +
                      toDisplayString(
                        ((w = unref(l)) == null ? void 0 : w.length) || 0,
                      ),
                    1,
                  ),
                ]),
                                ...
                                createElementBlock(
                                  'li',
                                  {
                                    ...
                                    class: normalizeClass({
                                      ...
                                      playing:
                                        unref(i) && e.value.currentIndex === h,
                                    }),
                                    onClick: (J) => d(h),
                                  },
                                  [
